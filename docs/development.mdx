...
<AccordionGroup>
  <Accordion icon="code" title="Node.js Setup">
    Install Node.js 18+ and npm for JavaScript/TypeScript development: ```bash #
    Install Node.js (using nvm recommended) curl -o-
    https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.0/install.sh | bash nvm
    install 18 nvm use 18 # Verify installation node --version npm --version ```
  </Accordion>
  <Accordion icon="code" title="Python Setup">
    For Python implementations, use Python 3.8+: ```bash # Install Python (using
    pyenv recommended) pyenv install 3.11.0 pyenv global 3.11.0 # Create virtual
    environment python -m venv haip-env source haip-env/bin/activate # On
    Windows: haip-env\Scripts\activate # Install dependencies pip install
    websockets asyncio ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion icon="server" title="HAIP Server">
    Run the complete HAIP server for development: ```bash # Clone HAIP server
    git clone https://github.com/haiprotocol/haip cd haip/haip-server # Install
    dependencies npm install # Build the project npm run build # Start server
    npm run dev ``` The server will be available at: - **WebSocket**:
    ...
    `http://localhost:8080/stats` See the [HAIP Server
    documentation](/server/overview) for complete setup and configuration.
  </Accordion>
  <Accordion icon="wrench" title="HAIP CLI">
    Use the HAIP CLI for testing and debugging: ```bash # Install HAIP CLI npm
    install -g @haip/cli # Test connection haip connect
    ws://localhost:8080/haip/websocket # Send test message haip send-text
    "Hello, agent!" # Monitor events haip monitor ```
  </Accordion>
</AccordionGroup>
...
<Accordion icon="vial" title="Test HAIP Messages">
  ...
  describe('HAIPClient', () => {
    ...
    it('should handle text messages correctly', () => {
      ...
      expect(mockWs.send).toHaveBeenCalledWith(
        expect.stringContaining('"type":"MESSAGE_START"')
      );
      expect(mockWs.send).toHaveBeenCalledWith(
        expect.stringContaining('"type":"MESSAGE_END"')
      );
    });
  });
  ```
