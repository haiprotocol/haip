...
describe("HAIPClient", () => {
  ...
  describe("Text Messaging", () => {
    test("should send text message", async () => {
      ...
      const sentMessages = mockTransport.getSentMessages();
      const startMessage = sentMessages.find(m => m.type === "TEXT_MESSAGE_START");
      const partMessage = sentMessages.find(m => m.type === "TEXT_MESSAGE_PART");
      const endMessage = sentMessages.find(m => m.type === "TEXT_MESSAGE_END");
