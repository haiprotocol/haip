...
export class HAIPClientImpl extends EventEmitter implements HAIPClient {
  ...
  private createTransport(): HAIPTransport {
    ...
    switch (transportConfig.type) {
      case "websocket":
        return new WebSocketTransport(transportConfig.url, transportConfig.token, transportConfig.options);
      case "sse":
  ...
  private initializeFlowControl(): void {
    ...
    channels.forEach(channel => {
      ...
      this.state.byteCredits.set(channel, this.config.flowControl?.initialCreditBytes || 1024 * 1024);
  ...
  private handleRunFinished(payload: HAIPRunFinishedPayload, message: HAIPMessage): void {
    ...
    if (runId) {
      ...
      if (run) {
        run.status = payload.status === "OK" ? "finished" : payload.status === "CANCELLED" ? "cancelled" : "error";
  ...
  private scheduleReconnection(): void {
    ...
    this.logger.info(`Scheduling reconnection in ${delay}ms (attempt ${this.state.reconnectAttempts + 1})`);
  ...
  async disconnect(): Promise<void> {
    ...
    // Prevent reconnection attempts during manual disconnect
  ...
  async finishRun(runId: string, status?: "OK" | "CANCELLED" | "ERROR", summary?: string): Promise<void> {
  ...
  async listTools(channel: HAIPChannel, tools: Array<{ name: string; description?: string }>): Promise<void> {
