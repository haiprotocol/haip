...
import { HAIPMessage, HaipToolClient, HAIPToolSchema } from "haip";
...
server.on("textMessage", (sessionId, message) => {
    ...
    const echoMessage = HAIPServerUtils.createTextMessageStart(sessionId, {
...
function generateTestToken(): string {
    ...
    const signature = Buffer.from("test-secret-key")
        .toString("base64")
        .replace(/\+/g, "-")
        .replace(/\//g, "_")
        .replace(/=/g, "");
...
console.log(
    'HTTP Stream: curl -X POST http://localhost:8080/haip/stream -H "Authorization: Bearer test-token"'
);
