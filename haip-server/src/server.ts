...
export class HAIPServer extends EventEmitter {
      ...
      ws.on('message', (data: WebSocket.Data) => {
        try {
          ...
          if (typeof data === 'string') {
            ...
            // Convert Buffer to string - it might be JSON text
  ...
  private handleMessage(sessionId: string, message: any): void {
    ...
    switch (message.type) {
      ...
      default:
        ...
        this.sendError(sessionId, 'UNSUPPORTED_TYPE', `Unknown message type: ${message.type}`);
    }
  }
  ...
  private handleBinaryData(sessionId: string, data: Buffer): void {
    ...
    // Handle binary data (audio chunks, etc.)
  ...
  private handlePing(sessionId: string, message: HAIPMessage): void {
    console.log('handlePing called for session:', sessionId, 'with nonce:', message.payload.nonce);
  ...
  private sendToolUpdate(sessionId: string, callId: string, status: string, progress?: number): void {
  ...
  private handleDisconnect(sessionId: string): void {
    ...
    // Clear heartbeat interval for the disconnected session
  ...
  public stop(): void {
    ...
    this.wss.close(() => {
      // Close HTTP server after WebSocket server is closed
      this.server.close(() => {
        if (this.config.enableLogging) {
