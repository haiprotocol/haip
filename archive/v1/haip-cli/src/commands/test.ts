...
function validateResponse(message: any): boolean {
    return (
        message && message.id && message.type && message.payload && typeof message.ts === "string"
    );
}
...
function displayTestResults(results: TestResults): void {
    ...
    console.log(
        `  Bytes/sec: ${chalk.white(HAIPCLIUtils.formatBytes(results.throughput.bytesPerSecond))}`
    );
