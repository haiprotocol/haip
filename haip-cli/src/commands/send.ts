      ...
      try {
        ...
        connection.on("connected", async () => {
          try {
            ...
            console.error(chalk.red(`Failed to send message: ${error instanceof Error ? error.message : String(error)}`));
            await connection.disconnect();
            process.exit(1);
          }
        });
      ...
      try {
        ...
        connection.on("connected", async () => {
          try {
            ...
            console.error(chalk.red(`Failed to send tool call: ${error instanceof Error ? error.message : String(error)}`));
            await connection.disconnect();
            process.exit(1);
          }
        });
  ...
  return new Command("run")
    ...
    .action(async (options: any) => {
      try {
        ...
        connection.on("connected", async () => {
          try {
            const runMessage = HAIPCLIUtils.createRunStartMessage(options.threadId, metadata);
            ...
            console.error(chalk.red(`Failed to start run: ${error instanceof Error ? error.message : String(error)}`));
            await connection.disconnect();
            process.exit(1);
          }
        });
