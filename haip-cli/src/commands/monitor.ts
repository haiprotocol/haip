    ...
    .action(async (options: any) => {
      try {
        ...
        const monitorOptions: HAIPCLIMonitorOptions = {
          ...
          filterChannels: options.filterChannels ? options.filterChannels.split(",") : undefined,
        ...
        console.error(chalk.red(`Failed to start monitoring: ${error instanceof Error ? error.message : String(error)}`));
        process.exit(1);
      }
    });
}
